"""
Canónica mensual REAL histórica completa: MV desde v_trips_real_canon (sin ventana 120d).
Para Resumen mensual: trips, revenue/margin, active_drivers_core por (month_start, country).
Sustituye funcionalmente la dependencia de real_drill_dim_fact + v_real_trip_fact_v2 (120d) para Resumen.
NO toca batch de segmentación ni legacy.
"""
from alembic import op

revision = "107_real_monthly_canonical_hist_mv"
down_revision = "106_real_driver_segmentation_canonical"
branch_labels = None
depends_on = None

# Misma lógica país que v_real_trip_fact_v2 (099): park_name_raw -> co/pe/''.
# Fuente: ops.v_trips_real_canon (trips_all < 2026 + trips_2026 >= 2026), cobertura histórica completa.
SQL_MV = """
...
with_geo AS (
    SELECT
        wp.id,
        wp.fecha_inicio_viaje,
        wp.condicion,
        wp.conductor_id,
        wp.comision_empresa_asociada,
        CASE
...
base AS (
    SELECT
        ...
        conductor_id,
        ABS(COALESCE(comision_empresa_asociada, 0)) AS margin_total
    FROM with_geo
)
SELECT
    month_start,
...
def upgrade() -> None:
    ...
    op.execute(SQL_MV)
    op.execute("""
        COMMENT ON MATERIALIZED VIEW ops.mv_real_monthly_canonical_hist IS
        'Canónica mensual REAL histórica. Fuente: v_trips_real_canon (sin 120d). Grano: month_start, country. trips/margin_total solo completados; active_drivers_core = COUNT(DISTINCT conductor_id) completados. Refrescar tras cambios en trips_all/trips_2026.'
    """)
    op.execute("""
        CREATE UNIQUE INDEX uq_mv_real_monthly_canonical_hist
        ON ops.mv_real_monthly_canonical_hist (month_start, country)
    """)
