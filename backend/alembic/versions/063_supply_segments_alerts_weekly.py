"""
Supply PRO: segmentación semanal por conductor, agregados por segmento, anomalías y alertas.
Fuente: ops.mv_driver_weekly_stats. Geo: dim.v_geo_park (fallback dim.dim_park + UNKNOWN).
Incluye: mv_driver_segments_weekly, mv_supply_segments_weekly, mv_supply_segment_anomalies_weekly,
mv_supply_alerts_weekly, v_supply_alert_drilldown, ops.refresh_supply_alerting_mvs().
"""
from alembic import op

revision = "063_supply_segments_alerts"
down_revision = "062_conductor_nombre"
branch_labels = None
depends_on = None

# Segmentos por trips_completed_week: FT>=60, PT 20-59, CASUAL 5-19, OCCASIONAL 1-4, DORMANT 0
_SEGMENT_CASE = """
    CASE
...
def upgrade() -> None:
    # ----- FASE 1: MV Driver Segments Weekly (por driver) -----
    ...
    # ----- FASE 2: MV Supply Segments Weekly (agregado por segmento) -----
    ...
    # ----- FASE 3: MV Segment Anomalies Weekly -----
    ...
    # ----- FASE 5: Vista drilldown (filtros vía WHERE por el cliente) -----
    ...
    # ----- FASE 6: Función refresh -----
    op.execute("""
        CREATE OR REPLACE FUNCTION ops.refresh_supply_alerting_mvs()
        RETURNS void
        LANGUAGE plpgsql
        AS $$
        BEGIN
...
def downgrade() -> None:
    op.execute("DROP FUNCTION IF EXISTS ops.refresh_supply_alerting_mvs() CASCADE")
    op.execute("DROP VIEW IF EXISTS ops.v_supply_alert_drilldown CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS ops.mv_supply_alerts_weekly CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS ops.mv_supply_segment_anomalies_weekly CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS ops.mv_supply_segments_weekly CASCADE")
