"""
050_drill_calendar_fix: Calendario completo + Park fix + Unit Economics + B2B + robustez.
- ops.v_real_trips_base_drill: base desde trips_all LEFT JOIN parks (lob_group, segment_tag, park_name_resolved, park_bucket).
- ops.v_real_data_coverage: last_trip_date, min_trip_date, last_month_with_data, last_week_with_data por país.
- ops.v_real_drill_country_month/week: calendario completo hasta periodo actual, estado CERRADO/ABIERTO/FALTA_DATA/VACIO.
- ops.v_real_drill_lob_month/week: drill por LOB desde base.
- ops.v_real_drill_park_month/week: drill por park con park_name_resolved, park_bucket (SIN_PARK_ID, PARK_NO_CATALOG, OK).
Fuente: public.trips_all. Sin dependencia de MVs.
"""
from alembic import op

revision = "050_drill_calendar_fix"
down_revision = "049_drill_calendar"
branch_labels = None
depends_on = None


def upgrade() -> None:
    # 1) Vista base: trips_all LEFT JOIN parks + lob_group + segment_tag + park_name_resolved + park_bucket
    op.execute("DROP VIEW IF EXISTS ops.v_real_trips_base_drill CASCADE")
    op.execute("""
        ...
        city_key AS (
            SELECT
                ...
                CASE
                    WHEN v.park_catalog_name::text ILIKE '%%cali%%' OR v.park_city::text ILIKE '%%cali%%' THEN 'cali'
                    WHEN v.park_catalog_name::text ILIKE '%%bogot%%' OR v.park_city::text ILIKE '%%bogot%%' THEN 'bogota'
                    WHEN v.park_catalog_name::text ILIKE '%%barranquilla%%' OR v.park_city::text ILIKE '%%barranquilla%%' THEN 'barranquilla'
                    WHEN v.park_catalog_name::text ILIKE '%%medell%%' OR v.park_city::text ILIKE '%%medell%%' THEN 'medellin'
                    WHEN v.park_catalog_name::text ILIKE '%%cucut%%' OR v.park_city::text ILIKE '%%cucut%%' THEN 'cucuta'
                    WHEN v.park_catalog_name::text ILIKE '%%bucaramanga%%' OR v.park_city::text ILIKE '%%bucaramanga%%' THEN 'bucaramanga'
                    WHEN v.park_catalog_name::text ILIKE '%%lima%%' OR v.park_city::text ILIKE '%%lima%%' OR TRIM(COALESCE(v.park_catalog_name::text,'')) = 'Yego' THEN 'lima'
                    WHEN v.park_catalog_name::text ILIKE '%%arequip%%' OR v.park_city::text ILIKE '%%arequip%%' THEN 'arequipa'
                    WHEN v.park_catalog_name::text ILIKE '%%trujill%%' OR v.park_city::text ILIKE '%%trujill%%' THEN 'trujillo'
                    ELSE LOWER(TRIM(COALESCE(v.park_city::text, '')))
                END AS city_norm
            FROM with_norm v
        )
        SELECT
            ...
            v.comision_empresa_asociada,
            v.distancia_km,
            COALESCE(NULLIF(TRIM(v.park_catalog_name::text), ''), 'PARK ' || COALESCE(v.park_id::text, 'NULL')) AS park_name_resolved,
            CASE
                WHEN v.park_id IS NULL THEN 'SIN_PARK_ID'
    ...
    # 2) Vista de cobertura por país (desde base)
    ...
    # 3) Country MONTH con calendario completo y estado
    ...
    # 4) Country WEEK con calendario completo y estado
    ...
    # 5) LOB MONTH (agregado desde base, sin segment_tag en agrupación para "Todos")
    ...
    op.execute("""
        ...
        SELECT
            ...
            SUM(COALESCE(distancia_km::numeric, 0)) / 1000.0 AS distance_total_km,
            CASE WHEN COUNT(*) > 0 THEN (SUM(COALESCE(distancia_km::numeric, 0)) / 1000.0) / COUNT(*) ELSE NULL END AS distance_km_avg,
            MAX(trip_ts) AS last_trip_ts,
    ...
    # 7) Park MONTH con park_name_resolved, park_bucket (incluye SIN_PARK_ID)
