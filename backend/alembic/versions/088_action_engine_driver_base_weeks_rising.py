"""
Action Engine — Add weeks_rising_consecutively to v_action_engine_driver_base for explainability (persistence).
Non-destructive: only adds column to SELECT.
"""
from alembic import op

revision = "088_action_engine_driver_base_weeks_rising"
down_revision = "087_top_driver_behavior_views"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute("""
        CREATE VIEW ops.v_action_engine_driver_base AS
        WITH src AS (
        ...
        SELECT
            ...
            active_weeks_in_window, weeks_declining_consecutively, weeks_rising_consecutively,
            cohort_type
        FROM tagged
        WHERE cohort_type IS NOT NULL
    """)
    op.execute("""
        CREATE VIEW ops.v_action_engine_cohorts_weekly AS
        WITH cohort_meta AS (
    ...
    """)
    op.execute("""
        CREATE VIEW ops.v_action_engine_recommendations_weekly AS
        SELECT
            c.*,
...
def downgrade() -> None:
    ...
    op.execute("DROP VIEW IF EXISTS ops.v_action_engine_driver_base CASCADE")
    # Recreate 086-style view (without weeks_rising_consecutively) and cohorts, recommendations
    op.execute("""
        CREATE VIEW ops.v_action_engine_driver_base AS
        WITH src AS (SELECT * FROM ops.mv_driver_behavior_alerts_weekly),
        tagged AS (
            SELECT s.*,
                CASE
                    WHEN s.segment_current IN ('FT', 'ELITE', 'LEGEND') AND s.risk_band IN ('high risk', 'medium risk') AND s.delta_pct IS NOT NULL AND s.delta_pct < -0.15 THEN 'high_value_deteriorating'
                    WHEN s.weeks_declining_consecutively >= 3 AND s.alert_type NOT IN ('Critical Drop', 'Moderate Drop') THEN 'silent_erosion'
                    WHEN s.segment_current IN ('CASUAL', 'PT') AND (s.alert_type = 'Strong Recovery' OR (s.delta_pct IS NOT NULL AND s.delta_pct > 0 AND s.risk_band IN ('stable', 'monitor'))) THEN 'recoverable_mid_performers'
                    WHEN s.segment_current IN ('CASUAL', 'PT') AND s.movement_type = 'upshift' AND s.delta_pct IS NOT NULL AND s.delta_pct > 0 THEN 'near_upgrade_opportunity'
                    WHEN (s.segment_current IN ('FT', 'ELITE', 'LEGEND', 'PT') AND s.movement_type IN ('downshift', 'drop')) OR (s.delta_pct IS NOT NULL AND s.delta_pct < -0.10 AND s.risk_band IN ('medium risk', 'monitor') AND s.segment_current IN ('FT', 'ELITE', 'LEGEND', 'PT')) THEN 'near_drop_risk'
                    WHEN s.alert_type = 'High Volatility' THEN 'volatile_drivers'
                    WHEN s.avg_trips_baseline >= 40 AND s.delta_pct IS NOT NULL AND s.delta_pct < -0.20 AND s.delta_pct > -0.50 AND s.segment_current IN ('FT', 'ELITE', 'LEGEND') THEN 'high_value_recovery_candidates'
                    ELSE NULL
                END AS cohort_type
            FROM src s
        )
    ...
    op.execute("""
        ...
        agg AS (
            SELECT d.week_start, d.week_label, d.cohort_type,
                COUNT(*)::int AS cohort_size,
                ROUND(AVG(d.risk_score)::numeric, 2) AS avg_risk_score,
                ROUND(AVG(d.delta_pct)::numeric, 4) AS avg_delta_pct,
                ROUND(AVG(d.avg_trips_baseline)::numeric, 2) AS avg_baseline_value,
        ...
        SELECT a.week_start, a.week_label, a.cohort_type, a.cohort_size, a.avg_risk_score, a.avg_delta_pct, a.avg_baseline_value, a.dominant_segment,
            m.suggested_priority, m.suggested_channel, m.action_name, m.action_objective
        FROM agg a
        JOIN cohort_meta m ON m.cohort_type = a.cohort_type
    """)
    op.execute("""
        CREATE VIEW ops.v_action_engine_recommendations_weekly AS
        SELECT c.*,
            (c.cohort_size * 0.4 + COALESCE(c.avg_risk_score, 0) * 0.4 + CASE WHEN c.dominant_segment IN ('FT', 'ELITE', 'LEGEND') THEN 20 ELSE 0 END) AS priority_score
        FROM ops.v_action_engine_cohorts_weekly c
    """)
