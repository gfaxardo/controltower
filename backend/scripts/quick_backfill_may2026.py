"""
Script liviano: poblar FACT_DAILY para May 2026 directamente desde public.trips_2026.
Evita el enriched view (UNION+DISTINCT ON pesado) — solo trips_2026 tiene datos de May.
"""
...
SELECT
    ...
    CASE
        ...
        THEN ROUND(
            100.0 * COUNT(*) FILTER (WHERE t.completed_flag AND t.revenue_yego_net IS NOT NULL)
            / COUNT(*) FILTER (WHERE t.completed_flag), 2
        )
        ELSE NULL
...
FROM (
    ...
    WHERE fecha_inicio_viaje::date >= '2026-05-01'
      AND fecha_inicio_viaje::date <= '2026-05-31'
) t
        ...
        WHERE lower(trim(dp.park_id::text)) = lower(trim(rl.park_id::text))
          AND (
              ...
              OR (rl.rule_type = 'park_plus_tipo_servicio'
                  AND rl.tipo_servicio_values IS NOT NULL)
              OR (rl.rule_type = 'park_plus_works_terms'
                  AND rl.works_terms_values IS NOT NULL
                  AND EXISTS (
                      SELECT 1 FROM unnest(rl.works_terms_values) wv
                      WHERE ops.normalized_works_terms(d.works_terms::text)
                         = ops.normalized_works_terms(wv::text)
                  ))
          )
    ),
    best AS (
        SELECT DISTINCT ON (spec_score) * FROM m ORDER BY spec_score DESC, is_subfleet ASC, COALESCE(parent_fleet_name, '') ASC
    )
    SELECT business_slice_name, fleet_display_name, is_subfleet, subfleet_name, parent_fleet_name FROM best LIMIT 1
) r ON TRUE
GROUP BY
    t.trip_date, dp.country, dp.city,
...
ON CONFLICT (
    ...
    COALESCE(city, ''::text),
    business_slice_name,
    COALESCE(fleet_display_name, ''::text),
    is_subfleet,
    COALESCE(subfleet_name, ''::text),
    COALESCE(parent_fleet_name, ''::text)
)
DO UPDATE SET
...
def main():
    print("Iniciando carga directa de FACT_DAILY para May 2026...")
    ...
    with get_db_audit(timeout_ms=3_600_000) as conn:
        ...
        print("  -> Borrando filas existentes de May 2026...")
        cur.execute("DELETE FROM ops.real_business_slice_day_fact WHERE trip_date >= '2026-05-01' AND trip_date <= '2026-05-31'")
        deleted = cur.rowcount
        conn.commit()
        print(f"     {deleted} filas eliminadas en {time.perf_counter() - t_pre:.1f}s")

        print("  -> Insertando nuevos datos (puede tomar varios minutos)...")
        t_ins = time.perf_counter()
        cur.execute(SQL_DAY_INSERT)
        inserted = cur.rowcount
        conn.commit()
        dt = time.perf_counter() - t_ins
        print(f"     {inserted} filas insertadas en {dt:.1f}s")

        print(f"\n  TOTAL: {time.perf_counter() - t0:.1f}s")
        cur.close()

    if inserted > 0:
        print(f"\nOK. FACT_DAILY ahora tiene {inserted} filas para May 2026.")
        return 0
    else:
        print("\nERROR: 0 filas insertadas. Revisar SQL.")
        return 1
