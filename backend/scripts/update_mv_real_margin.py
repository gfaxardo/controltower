#!/usr/bin/env python3
"""
Actualiza MV real para agregar margen_unitario_yego.
Crea ops.mv_real_trips_monthly_v4 y hace swap seguro sin CASCADE.
"""
import sys
import os
...
def update_mv_real_margin():
    print("=" * 80)
    print("ACTUALIZANDO MV REAL: MARGEN UNITARIO YEGO")
    print("=" * 80)
    ...
    try:
        with get_db() as conn:
            ...
            print("\n1. Creando ops.mv_real_trips_monthly_v4...")
            cursor.execute("""
                CREATE MATERIALIZED VIEW ops.mv_real_trips_monthly_v4 AS
                ...
                SELECT 
                    ...
                    CASE
                        ...
                        THEN (-1 * SUM(commission_yego_signed)) / SUM(trips_real_completed)
                        ELSE NULL
            ...
            """)
            conn.commit()
            print("   [OK] MV v4 creada")

            print("\n2. Creando indices en v4...")
            indices = [
                ("idx_mv_real_trips_monthly_v4_month", "month"),
                ("idx_mv_real_trips_monthly_v4_country_city_lob_seg_month", "country, city_norm, lob_base, segment, month"),
                ("idx_mv_real_trips_monthly_v4_country", "country"),
                ("idx_mv_real_trips_monthly_v4_city_norm", "city_norm")
            ]
            for idx_name, cols in indices:
                cursor.execute(f"""
                    CREATE INDEX IF NOT EXISTS {idx_name}
                    ON ops.mv_real_trips_monthly_v4({cols});
                """)
                print(f"   [OK] Indice {idx_name} creado")
            conn.commit()

            print("\n3. Poblando MV v4...")
            cursor.execute("REFRESH MATERIALIZED VIEW ops.mv_real_trips_monthly_v4")
            conn.commit()
            print("   [OK] MV v4 poblada")

            print("\n4. Swap seguro (sin CASCADE)...")
            cursor.execute("BEGIN")
            cursor.execute("""
                ALTER MATERIALIZED VIEW ops.mv_real_trips_monthly
                RENAME TO mv_real_trips_monthly_old_margin;
            """)
            cursor.execute("""
                ALTER MATERIALIZED VIEW ops.mv_real_trips_monthly_v4
...
if __name__ == "__main__":
    sys.exit(update_mv_real_margin())
