        ...
        <div className="h-11 px-4 flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {authRequired && username && (
              <span className="text-2xs text-ct-nav-text/70 max-w-[8rem] truncate hidden md:inline" title={username}>{username}</span>
            )}
            {authRequired && (
              <button type="button" onClick={() => { logout(); navigate('/login', { replace: true }) }}
                className="px-2 py-1 rounded text-2xs text-ct-nav-text/70 hover:text-white hover:bg-white/10 transition-colors">
                Salir
              </button>
            )}
            <button type="button" onClick={() => setShowAdminModal(true)}
              className="px-2 py-1 rounded text-2xs text-ct-nav-text/70 hover:text-white hover:bg-white/10 transition-colors flex items-center gap-1">
                  ...
                  <>
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      <button type="button" onClick={() => setPlanValidacionInner('out_of_universe')}
                        className={`px-2.5 py-1 rounded text-2xs font-medium transition-all ${planValidacionInner === 'out_of_universe' ? 'bg-ct-accent text-white shadow-sm' : 'text-ct-text2 hover:text-ct-text hover:bg-ct-border'}`}>Expansión</button>
                      <button type="button" onClick={() => setPlanValidacionInner('missing')}
                        className={`px-2.5 py-1 rounded text-2xs font-medium transition-all ${planValidacionInner === 'missing' ? 'bg-ct-accent text-white shadow-sm' : 'text-ct-text2 hover:text-ct-text hover:bg-ct-border'}`}>Huecos</button>
                    </div>
