...
import { MATRIX_KPIS, computeDeltas } from './omniview/omniviewMatrixUtils.js'
        ...
        <table className="border-collapse min-w-full" style={{ tableLayout: 'fixed', width: 'auto' }}>
          ...
          <tbody>
            {cityEntries.map(([cityKey, cityData]) => {
              ...
              const lineEntries = [...cityData.lines.entries()].sort((a, b) =>
                a[1].business_slice_name.localeCompare(b[1].business_slice_name)
              )
...
function CityBlock ({
  ...
  return (
    <>
      <tr
        className="bg-slate-100 hover:bg-slate-200/80 cursor-pointer transition-colors border-t-2 border-slate-300"
        onClick={onToggle}
      >
        <td
          colSpan={2}
          className={`sticky left-0 z-10 bg-slate-100 px-2 ${py} ${fontSize} font-bold text-slate-700 uppercase tracking-wide select-none`}
          style={{ minWidth: COL1_W + COL2_W }}
        >
          <span className="inline-block w-3.5 text-center mr-1 text-slate-400 text-[10px]">
            {isCollapsed ? '▶' : '▼'}
          </span>
          {cityData.city}
      ...
      {!isCollapsed &&
        lineEntries.map(([lineKey, lineData]) => (
          <LineRow
            key={lineKey}
            cityKey={cityKey}
            lineKey={lineKey}
...
function LineRow ({
  ...
  const deltas = useMemo(
    () => computeDeltas(lineData.periods, allPeriods),
    [lineData.periods, allPeriods]
  )
  ...
  return (
    <tr className={`border-b border-gray-100/80 ${isSubfleet ? 'bg-gray-50/40' : 'bg-white'} hover:bg-blue-50/40 transition-colors`}>
      <td
        className={`sticky left-0 z-10 bg-inherit border-r border-gray-100 ${py}`}
        style={{ width: COL1_W, minWidth: COL1_W }}
      />

      <td
        className={`sticky z-10 bg-inherit px-2 ${py} ${fontSize} text-gray-700 font-medium border-r border-gray-200 whitespace-nowrap overflow-hidden text-ellipsis`}
      ...
      >
        ...
        <span className={isSubfleet ? 'text-gray-500' : 'text-gray-800'}>
