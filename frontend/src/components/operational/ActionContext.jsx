  ...
  return (
    <div className="rounded-lg border border-blue-100 bg-gradient-to-r from-blue-50 to-white px-3 py-2.5 shadow-sm">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
        <span className="text-xs font-semibold uppercase tracking-wider text-blue-400">Contexto</span>
        ...
        {selection && onDrill && (
          <button
            ...
            className="ml-auto px-2.5 py-1 rounded text-xs font-semibold bg-blue-600 text-white hover:bg-blue-700 transition-colors"
          >
            Drill
          </button>
        )}
        ...
        {relatedMetrics.length > 0 && (
          <div className="w-full flex flex-wrap gap-1 mt-1 pt-1.5 border-t border-blue-100">
            <span className="text-xs font-medium text-blue-400 uppercase tracking-wider">Relacionado</span>
            {relatedMetrics.map((m, i) => (
              <span key={i} className="text-xs text-gray-600 bg-white border border-gray-200 rounded px-1.5 py-0.5">
...
function ContextPill ({ label, value, onClick, plain }) {
  const cls = plain
    ? 'inline-flex items-center gap-0.5 text-xs text-gray-500 bg-white border border-gray-150 rounded px-1.5 py-0.5'
    : 'inline-flex items-center gap-0.5 text-xs text-blue-700 bg-white border border-blue-200 rounded px-1.5 py-0.5 hover:bg-blue-100 cursor-pointer transition-colors'
